...
if(state == LinphoneConfiguringFailed && strcmp(message, "http error") == 0){
ms_error("Provisioning URI request failed");
_linphone_core_read_config(lc);
linphone_core_start(lc);
}
...
// Lock CPU and Wifi, so we get decent performance. Maybe cpu is not necessary.

	...
	for (cur_attr = (xmlNode *)element->attributes; cur_attr; cur_attr = cur_attr->next) {
		...
		if(strcmp((const char*)cur_attr->name, "name") == 0) {
			...
			if(strcmp((const char*)cur_attr->children->content, "true") == 0) {
				ms_message("overwrite - sectionName: %s, key: %s = true", sectionName, name);
				linphone_config_set_overwrite_flag_for_entry(ctx->lpc, sectionName, name, true);
				overwrite = TRUE;
			} else{
				linphone_config_set_overwrite_flag_for_entry(ctx->lpc, sectionName, name, false);
			}
		}
	}
...
static int processSection(xmlElement *element, xml2lpc_context *ctx) {
	...
	for (cur_attr = (xmlNode *)element->attributes; cur_attr; cur_attr = cur_attr->next) {
		...
		if(strcmp((const char*)cur_attr->name, "name") == 0) {
			...
			if(strcmp((const char*)cur_attr->children->content, "true") == 0) {
				ms_message("overwriteSection - sectionName: %s, = true", name);
				linphone_config_set_overwrite_flag_for_section(ctx->lpc, name, true);
			} else{
				linphone_config_set_overwrite_flag_for_section(ctx->lpc, name, false);
			}
		}//Marcus end
	}
